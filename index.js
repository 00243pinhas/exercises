...
// here is the function for loading the page with javascript 


function displalyMenueItems(menuitme){
  let displaymenu = menuitme.map(function(item){
...
// du coup on se rend compte que le code est hardly written 
// donc nous devons dynamiser notre  code en sorte que si on ajoute un element sur l'objet 
// que le code ne bug pas 

const btnContenaire = document.querySelector('.btn-container')


function displaymenubtns(){
  
  const missingCategory= menu.reduce(function(acc,curr){
    if (!acc.includes(curr.category)){
      acc.push(curr.category)
    }
    return acc 
  },['all'])
  // here we used the reduced method to loop and include the missing element 
  //console.log(missingCategory)

  const mapLesbtn= missingCategory.map(function(btn){
    return ` <button type="button" class="filter-btn"
     data-id="${btn}">${btn}</button>`
  }).join('')
 
  btnContenaire.innerHTML=mapLesbtn
...
filterBtn.forEach(function(btn){
  btn.addEventListener('click',function(e){
    ...
    const menucat = menu.filter (function(menueitem){
      if (menueitem.category===categori){
        return menueitem
      }
    });
